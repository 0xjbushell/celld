...
/// Admission order for cells waiting on resident capacity.
///
/// A hibernated cell whose node is at its resident ceiling must wait for a
/// slot. celld's executor waits on a `Notify`: every waiter wakes on each
/// release and races to reserve. That is unfair by construction, and the
/// 2026-08-02 working-set pilot measured the consequence — 13 requests
/// timed out after 20 s while roughly 2,336 slots were freed around them,
/// at 117 evictions/s. They never ran out of capacity; they lost every race.
///
/// Admission is therefore a QUEUE decision, and a pure one: given the
/// waiters in arrival order and the slots available, who is admitted?
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Waiter {
    /// Arrival tick — smaller is older.
    pub arrived_at: u64,
    pub id: u64,
}

/// Admit up to `slots` waiters, oldest first. Returns their ids.
///
/// FIFO is the whole policy. It costs nothing over a race and converts
/// "eventually, probably" into a bound: a waiter with `k` waiters ahead of
/// it is admitted within `k` releases, so no arrival pattern can starve it.
pub fn admit(waiters: &[Waiter], slots: usize) -> Vec<u64> {
    let mut order: Vec<&Waiter> = waiters.iter().collect();
    order.sort_by_key(|w| (w.arrived_at, w.id));
    order.into_iter().take(slots).map(|w| w.id).collect()
}

/// May one more cell be admitted to residency right now?
///
/// Two independent facts gate admission, and conflating them is what turned
/// the 2,000-ring lab workload into a cliff rather than a plateau.
///
