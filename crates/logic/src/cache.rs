...
//! a rename instead of a remote restore — measured 2026-08-02 at 46 serial
//! storage round trips saved per wake. The cost is that a node accumulates
//! one file per cell it has ever hosted, which is unbounded while the
//! durable population is unbounded.
//!
