...
mod tests {
    ...
    #[test]
    fn defaults_match_litestream_v0_5_16() {
        let levels = CompactionLevels::defaults();
        assert_eq!(levels.max_level(), 3);
        assert_eq!(levels.level(1).unwrap().interval, Duration::from_secs(30));
        assert_eq!(levels.level(2).unwrap().interval, Duration::from_secs(300));
        assert_eq!(levels.level(3).unwrap().interval, Duration::from_secs(3600));
        assert_eq!(levels.previous_level(SNAPSHOT_LEVEL), Some(3));
        assert_eq!(levels.next_level(3), Some(SNAPSHOT_LEVEL));
        assert_eq!(levels.next_level(SNAPSHOT_LEVEL), None);
    }

    #[test]
    fn validation_rejects_missing_reordered_and_invalid_intervals() {
        assert!(CompactionLevels::new(Vec::new()).is_err());
    ...
    #[test]
    fn interval_boundaries_match_litestream() {
        let level = DEFAULT_COMPACTION_LEVELS[1];
        let now = UNIX_EPOCH + Duration::from_secs(95);
        assert_eq!(
            level.previous_compaction_at(now),
            UNIX_EPOCH + Duration::from_secs(90)
        );
        assert_eq!(
            level.next_compaction_at(now),
