...
Credentials: S3 uses the standard AWS chain; gs:// uses Google Application\nDefault Credentials. az:// requires AZURE_STORAGE_ACCOUNT_NAME and exactly one\nCELLD_AZURE_AUTH mode: account-key (AZURE_STORAGE_ACCOUNT_KEY), sas\n(AZURE_STORAGE_SAS_KEY), managed-identity (optional AZURE_CLIENT_ID),\nworkload-identity (AZURE_TENANT_ID, AZURE_CLIENT_ID,\nAZURE_FEDERATED_TOKEN_FILE), client-secret (AZURE_TENANT_ID, AZURE_CLIENT_ID,\nAZURE_CLIENT_SECRET), or the explicit developer-tools opt-in (Azure CLI, then\nAzure Developer CLI). Entra identities require a container-scoped Storage Blob\nData Contributor role; management-plane roles do not authorize Blob data.\nAZURE_STORAGE_USE_EMULATOR=1 selects Azurite (using AZURITE_BLOB_STORAGE_URL);\nit rejects --endpoint, AZURE_STORAGE_ENDPOINT, and CELLD_AZURE_AUTH.\n\n\
...
mod tests {
    ...
    fn help_describes_each_supported_bucket_provider() {
        ...
        assert!(help.contains("CELLD_AZURE_AUTH mode: account-key"));
        assert!(help.contains("managed-identity"));
        assert!(help.contains("workload-identity"));
        assert!(help.contains("client-secret"));
        assert!(help.contains("developer-tools"));
        assert!(help.contains("AZURE_TENANT_ID"));
        assert!(help.contains("AZURE_CLIENT_ID"));
        assert!(help.contains("AZURE_FEDERATED_TOKEN_FILE"));
        assert!(help.contains("AZURE_CLIENT_SECRET"));
        assert!(help.contains("Storage Blob"));
        assert!(help.contains("Data Contributor"));
