...
static SVC_CALL_TX: OnceLock<tokio::sync::mpsc::UnboundedSender<SvcCallReq>> =
    OnceLock::new();
...
static SVC_RPC_TX: OnceLock<tokio::sync::mpsc::UnboundedSender<SvcRpcReq>> =
    OnceLock::new();
...
pub fn set_do_call_tx(tx: tokio::sync::mpsc::UnboundedSender<DoCallReq>) { let _ = DO_CALL_TX.set(tx); }
...
static PENDING_ARM_GATES: OnceLock<Mutex<HashMap<String, Vec<ArmGateRx>>>> =
    OnceLock::new();
...
fn spawn_arm_gate(cell: &str, at_ms: i64) {
    ...
    let Some(celld_logic::wake::Op::Put { key, due_ms }) =
        gate.flusher.arm_op(cell, at_ms)
    else {
        return;
    };
    ...
    let (tx, rx) = tokio::sync::oneshot::channel();
    {
        ...
        gates.retain_mut(|gate| {
            matches!(gate.try_recv(), Err(tokio::sync::oneshot::error::TryRecvError::Empty))
        });
        gates.push(rx);
    }
...
/// aborted mid-flight.
pub fn next_request_id() -> RequestId { next_do_request_id() }

static NEXT_DO_REQUEST_ID: AtomicU64 = AtomicU64::new(1);
static DO_REQUEST_PROCESS_PREFIX: OnceLock<u64> = OnceLock::new();
static DO_CALL_CANCELS:
    OnceLock<std::sync::Mutex<HashMap<RequestId, tokio::sync::oneshot::Sender<()>>>> =
    OnceLock::new();
fn do_call_cancels(
...
pub fn set_rpc_call_tx(tx: tokio::sync::mpsc::UnboundedSender<RpcCallReq>) { let _ = RPC_CALL_TX.set(tx); }
...
static HTTP_STREAMS: OnceLock<std::sync::Mutex<HashMap<u64, HttpStreamEntry>>> =
    OnceLock::new();
...
static RESPONSE_STREAM_WRITERS:
    OnceLock<std::sync::Mutex<HashMap<u64, ResponseStreamWriter>>> = OnceLock::new();
fn response_stream_writers(
) -> &'static std::sync::Mutex<HashMap<u64, ResponseStreamWriter>> {
    RESPONSE_STREAM_WRITERS.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}

pub type HttpChunkReceiver = tokio::sync::mpsc::Receiver<Result<Vec<u8>, String>>;
...
fn encode_http_response(mut response: HttpResponse, ws_target: bool) -> String {
    ...
    if let Some(stream) = response.stream.take() {
        ...
        register_http_stream(stream_id, HttpStreamSource::Receiver(stream));
...
pub struct HttpResponse {
    ...
    pub stream: Option<HttpChunkReceiver>,
    pub headers: Vec<(String, String)>,
...
pub enum WsOut { Text(String), Binary(Vec<u8>), Close(u16, String) }
...
pub fn ws_pull_register(id: u64, rx: WsPullReceiver) {
    ws_pull().lock().unwrap().insert(id, Arc::new(tokio::sync::Mutex::new(rx)));
}
...
pub fn ws_region_enter() {
    WS_PULL_REGION.with(|regions| regions.borrow_mut().push(Vec::new()));
}
...
pub fn ws_region_exit() {
    let opened = WS_PULL_REGION.with(|regions| {
        regions.borrow_mut().pop().unwrap_or_default()
    });
    for id in opened {
...
pub enum WsIn { Text(String), Binary(Vec<u8>) }
struct WsMeta {
    scope: String,
    hibernatable: bool,
    tags: Vec<String>,
...
fn decrement_regular_ws(scope: &str) {
    ...
    let Some(count) = counts.get_mut(scope) else { return };
...
pub fn ws_next_id() -> u64 { NEXT_WS_ID.fetch_add(1, Ordering::Relaxed) }
...
pub fn ws_register_outbound(id: u64, scope: &str) {
    ...
    if inserted { increment_regular_ws(scope); }
}
...
/// Last-touch per cell (any storage op or dispatch), read by the eviction
/// sweep. A resident cell idle past the threshold is hibernated.
static LAST_USE: OnceLock<std::sync::Mutex<HashMap<String, Instant>>> =
    OnceLock::new();
fn last_use() -> &'static std::sync::Mutex<HashMap<String, Instant>> {
    LAST_USE.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}
pub fn touch(scope: &str) {
    last_use().lock().unwrap().insert(scope.to_string(), Instant::now());
}
pub fn idle_secs(scope: &str) -> Option<u64> {
    last_use().lock().unwrap().get(scope).map(|t| t.elapsed().as_secs())
}
...
fn promise_store(id: u64, r: v8::Global<v8::PromiseResolver>) {
    PROMISES.with(|p| { p.borrow_mut().insert(id, r); });
}
...
fn resolve_res(tc: &mut v8::PinScope, id: u64, res: Result<asyncrt::OpOut, String>) {
    let g = match PROMISES.with(|p| p.borrow_mut().remove(&id)) { Some(g) => g, None => return };
    let r = v8::Local::new(tc, g);
    match res {
        Ok(asyncrt::OpOut::Str(v)) => { let s = v8::String::new(tc, &v).unwrap(); r.resolve(tc, s.into()); }
        Ok(asyncrt::OpOut::Bytes(b)) => { let v = bytes_value(tc, b); r.resolve(tc, v); }
        Err(e) => { let s = v8::String::new(tc, &e).unwrap(); let ex = v8::Exception::error(tc, s); r.reject(tc, ex); }
    }
}
...
fn handler_budget() -> Duration {
    ...
    *BUDGET.get_or_init(|| Duration::from_secs(
        std::env::var("CELLD_HANDLER_BUDGET_S").ok()
            .and_then(|value| value.parse().ok()).filter(|seconds| *seconds > 0)
            .unwrap_or(300),
    ))
}
...
static PENDING_ABORTS: OnceLock<std::sync::Mutex<std::collections::HashSet<RequestId>>> =
    OnceLock::new();
static PENDING_ABORT_COUNT: std::sync::atomic::AtomicUsize =
...
{
    let mut region: JoinSet<(u64, Result<asyncrt::OpOut, String>)> =
        JoinSet::new();
    ws_region_enter();
    ...
    let result = loop {
        ...
        if let Some(rx) = cell_rx {
            ...
            while service_reentrant(tc, rx)? {
                ...
                for (id, fut) in asyncrt::drain_spawns() {
                    let h = region
                        .spawn_on(async move { (id, fut.await) }, &asyncrt::op_handle());
                    idmap.insert(h.id(), id);
                }
            }
            if output.is_none()
                && !request_cancelled
                && take_request_cancellation(request_id)
            {
                request_cancelled = true;
                if let Some(id) = request_id {
                    let _ = abort_incoming_request(tc, id);
        ...
        if !request_cancelled && output.is_none() {
            if let Some(scope) = actor_scope {
                let alarm_at = *next_alarm_at
                    .get_or_insert_with(|| storage::get_alarm(scope));
        ...
        if region.is_empty() {
            ...
            if output.is_none() {
                if let (Some(rx), Some(Some(alarm_at))) = (cell_rx, next_alarm_at) {
                    ...
                    let until_alarm = Duration::from_millis(
        ...
        match joined {
            ...
            Ok(Some(Err(je))) => { // op panicked/aborted → reject its promise
    ...
    for id in idmap.values() { PROMISES.with(|p| { p.borrow_mut().remove(id); }); }
    for (id, _) in asyncrt::drain_spawns() { PROMISES.with(|p| { p.borrow_mut().remove(&id); }); }
    result
}
...
fn resolved_promise<'s>(
    ...
    let resolver = v8::PromiseResolver::new(tc)
        .ok_or_else(|| anyhow!("could not create event promise"))?;
...
fn complete_fetch_event<'s>(
    ...
    let result = promise.and_then(|promise| {
        run_loop_then(tc, promise, cell_rx, critical_scope, request_id, false, |tc, value| {
            if let Some(error) =
                critical_scope.and_then(storage::sql_critical_error)
            {
                return Err(anyhow!(error));
            }
            let response = read_response(tc, value, cell_rx)?;
...
fn run_event_value<'s>(
    ...
    let result = run_loop_then(
        ...
        true,
        |tc, value| {
        Ok((value, end_event_context(tc)?))
        },
    );
    match result {
        Ok(RunLoopOutcome::Complete(value)) => Ok(value),
        Ok(RunLoopOutcome::RequestCancelled) =>
            Err(anyhow!("The client has disconnected")),
        Err(error) => {
            let _ = end_event_context(tc);
            Err(error)
        }
    ...
    match job {
        CellJob::Fetch {
            ...
            let result = register_actor_name(tc, &scope, name.as_deref())
                .and_then(|()| {
                    call_dispatch_to(tc, &scope, FetchRequest {
                        url: &url,
                        method: &method,
                        body: &body,
                        headers: &headers,
        ...
        CellJob::Rpc { scope, name, method, args, reply } => {
            let result = register_actor_name(tc, &scope, name.as_deref())
                .and_then(|()| call_dispatch_rpc(tc, &scope, &method, args, rx));
            let _ = reply.send(result);
        }
        CellJob::WsOpen { scope, ws_id, protocol, reply } => {
            let result = call_dispatch_ws_open(tc, &scope, ws_id, &protocol, rx);
            let _ = reply.send(result);
        }
        CellJob::WsMessage { scope, ws_id, data, reply } => {
        ...
        CellJob::WsClosed {
            ...
            let result =
                call_dispatch_ws_closed(tc, &scope, ws_id, code, &reason, was_clean, rx);
            let _ = reply.send(result);
        }
        ...
        CellJob::AbortFetch { request_id } => {
            match abort_incoming_request(tc, request_id) {
                Ok(true) => {
...
fn service_reentrant(tc: &mut v8::PinScope,
                     rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<bool> {
...
fn call_fire_alarm(
    ...
    run_event_value(tc, ret, Some(rx), None, None)
        .map_err(AlarmRunError::user)?;
    Ok(())
}
...
fn call_dispatch_to(
    ...
    let f: v8::Local<v8::Function> = global.get(tc, key.into())
        .ok_or_else(|| anyhow!("no __dispatchTo"))?.try_into().map_err(|_| anyhow!("not fn"))?;
    ...
    let request_id_value: v8::Local<v8::Value> = match request.request_id {
        Some(request_id) => v8::String::new(tc, &request_id_string(request_id)).unwrap().into(),
...
fn call_dispatch_rpc(tc: &mut v8::PinScope, scope: &str, method: &str,
                     args: RpcData,
                     rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<RpcData> {
    let context = tc.get_current_context();
    let global = context.global(tc);
    let key = v8::String::new(tc, "__dispatchRpc").unwrap();
    let f: v8::Local<v8::Function> = global.get(tc, key.into())
        .ok_or_else(|| anyhow!("no __dispatchRpc"))?.try_into().map_err(|_| anyhow!("not fn"))?;
    let sc = v8::String::new(tc, scope).unwrap();
...
fn call_dispatch_ws(tc: &mut v8::PinScope, scope: &str, ws_id: u64, data: WsIn,
                    rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
    let context = tc.get_current_context();
    let global = context.global(tc);
    ...
    let f: v8::Local<v8::Function> = global.get(tc, key.into())
        ...
        .try_into().map_err(|_| anyhow!("not fn"))?;
    let sc = v8::String::new(tc, scope).unwrap();
...
fn call_dispatch_ws_open(tc: &mut v8::PinScope, scope: &str, ws_id: u64,
                         protocol: &str,
                         rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
    let context = tc.get_current_context();
    let global = context.global(tc);
    ...
    let f: v8::Local<v8::Function> = global.get(tc, key.into())
        .ok_or_else(|| anyhow!("no __wsOpen"))?.try_into().map_err(|_| anyhow!("not fn"))?;
    let sc = v8::String::new(tc, scope).unwrap();
    let id = v8::Number::new(tc, ws_id as f64);
...
fn call_dispatch_ws_closed(tc: &mut v8::PinScope, scope: &str, ws_id: u64,
                           code: u16, reason: &str, was_clean: bool,
                           rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
    let context = tc.get_current_context();
    let global = context.global(tc);
    let key = v8::String::new(tc, "__wsClosed").unwrap();
    let f: v8::Local<v8::Function> = global.get(tc, key.into())
        .ok_or_else(|| anyhow!("no __wsClosed"))?.try_into().map_err(|_| anyhow!("not fn"))?;
    let sc = v8::String::new(tc, scope).unwrap();
    let id = v8::Number::new(tc, ws_id as f64);
...
fn reject_reason(tc: &mut v8::PinScope, p: v8::Local<v8::Promise>) -> String {
    ...
    let stk = r.to_object(tc)
        .and_then(|o| { let k = v8::String::new(tc, "stack")?; o.get(tc, k.into()) })
        .map(|s| s.to_rust_string_lossy(tc)).unwrap_or_default();
    let tail = stk.lines().skip(1).take(3).map(|l| l.trim()).collect::<Vec<_>>().join(" <- ");
    if tail.is_empty() { msg } else { format!("{msg} [{tail}]") }
}

pub struct Engine;
impl Engine {
    pub fn init() {
        // Load FULL, 16-byte-aligned ICU data (matching v8-150's ICU 77) before
        // V8 init — the rusty_v8 prebuilt ships a reduced ICU that lacks regex
        // property-of-strings tables (e.g. /\p{RGI_Emoji}/v) real bundles use.
        if let Err(e) = v8::icu::set_common_data_77(deno_core_icudata::ICU_DATA) {
            tracing::warn!(code = e, "ICU set_common_data_77 failed");
        }
...
impl Worker {
    ...
    fn load_config_owned(
        ...
        let (context, fetch) = {
            ...
            module.instantiate_module(scope, resolve_external)
            ...
            let default = ns.get(scope, dk.into()).ok_or_else(|| anyhow!("no default export"))?
                .to_object(scope).ok_or_else(|| anyhow!("default not object"))?;
            let fk = v8::String::new(scope, "fetch").unwrap();
            let f: v8::Local<v8::Function> = default.get(scope, fk.into())
                .ok_or_else(|| anyhow!("no fetch"))?.try_into().map_err(|_| anyhow!("fetch not fn"))?;
                    ...
                    .and_then(|value| value.to_object(scope))
                {
                    ...
                    if let Some(handler) = default.get(scope, sk.into()) {
                        if handler.is_function() {
                            let key_ =
                                v8::String::new(scope, "selfScheduled")
                                    .unwrap();
    ...
    pub fn fire_alarm(&mut self, scope: &str, retry: i64,
                      rx: &std::sync::mpsc::Receiver<CellJob>)
                      -> std::result::Result<(), AlarmRunError> {
        let scheduled_time = storage::active_alarm_scheduled_time(scope)
            .ok_or_else(|| {
    ...
    pub fn dispatch_to_and_reply(
        ...
        touch(scope); // host guarantees residency before dispatching here
        let context_g = self.context.clone();
        v8::scope!(let hs, &mut self.isolate);
        let context = v8::Local::new(hs, context_g);
        let cs = &mut v8::ContextScope::new(hs, context);
        ...
        let global = context.global(tc);
        let key = v8::String::new(tc, "__dispatchTo").unwrap();
        let f: v8::Local<v8::Function> = match global.get(tc, key.into())
        ...
        complete_fetch_event(tc, ret, Some(rx), Some(scope), request.request_id, reply);
    }

    /// Invoke one public method on a DO instance (Cloudflare native RPC),
    /// with arguments and result in either `RpcData` flavor.
    pub fn dispatch_rpc_data(&mut self, scope: &str, method: &str,
                             args: RpcData,
                             rx: &std::sync::mpsc::Receiver<CellJob>)
                             -> Result<RpcData> {
        touch(scope);
    ...
    #[cfg(test)]
    pub fn dispatch_rpc(&mut self, scope: &str, method: &str, args: &str,
                        rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<String> {
    ...
    pub fn dispatch_entrypoint_rpc(
        ...
        let Some(ret) = f.call(
            tc,
            recv,
            &[entrypoint.into(), method.into(), args],
        ) else {
            let _ = end_event_context(tc);
        ...
        view_bytes(ret)
            .ok_or_else(|| anyhow!("entrypoint RPC answered non-bytes"))
    }

    pub fn dispatch_ws_open(&mut self, scope: &str, ws_id: u64, protocol: &str,
                            rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
        touch(scope);
        let context_g = self.context.clone();
        v8::scope!(let hs, &mut self.isolate);
    ...
    pub fn dispatch_ws(&mut self, scope: &str, ws_id: u64, data: WsIn,
                       rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
        touch(scope);
        let context_g = self.context.clone();
        v8::scope!(let hs, &mut self.isolate);
    ...
    pub fn dispatch_ws_closed(&mut self, scope: &str, ws_id: u64,
                              code: u16, reason: &str, was_clean: bool,
                              rx: &std::sync::mpsc::Receiver<CellJob>) -> Result<()> {
        touch(scope);
        let context_g = self.context.clone();
    ...
    fn fetch_and_reply_inner(
        ...
        complete_fetch_event(tc, ret, None, None, active_request_id, reply);
    ...
    fn worker_fetch_and_reply_inner(
        ...
        let built = match request.request_id {
            Some(_) => make_incoming_request(
                tc, request.url, request.method, request.body, request.headers,
            ),
            None => make_request(
                tc, request.url, request.method, request.body, request.headers,
            ),
        ...
        complete_fetch_event(tc, ret, Some(rx), None, active_request_id, reply);
        if let Some(request_id) = active_request_id {
            finish_incoming_request(tc, request_id);
        }
    }
...
fn op_svc_call_impl(
    ...
    let headers = serde_json::from_str(&args.get(4).to_rust_string_lossy(scope))
        .unwrap_or_default();
    let (tx, rx) = tokio::sync::oneshot::channel();
...
type LoaderRegistry =
    HashMap<u64, std::sync::mpsc::Sender<LoaderJob>>;
static LOADER_REGISTRY: OnceLock<std::sync::Mutex<LoaderRegistry>> =
    OnceLock::new();
...
fn op_loader_load(
    ...
    let loader_env = code.get("env").filter(|v| !v.is_null()).map(|v| v.to_string());
    ...
    std::thread::spawn(move || {
        ...
        for job in rx {
            match job {
                LoaderJob::Fetch { url, method, body, headers, reply } => {
                    if w.fetch_and_reply_id(
                        &url, &method, &body, &headers, None, reply,
                    )
                    .is_err()
                    {
                        break;
                ...
                LoaderJob::Rpc { entrypoint, method, args, reply } => {
                    let _ = reply.send(
...
fn drain_loader_load_error(
    rx: std::sync::mpsc::Receiver<LoaderJob>,
    error: &str,
) {
    for job in rx {
        match job {
...
fn op_loader_fetch(
    ...
    let method = args.get(2).to_rust_string_lossy(scope);
    let body = view_bytes(args.get(3)).unwrap_or_default();
    let headers = serde_json::from_str(
        &args.get(4).to_rust_string_lossy(scope),
    )
    .unwrap_or_default();
    let (tx, rx) = tokio::sync::oneshot::channel();
    let sent = loader_registry()
        ...
        .get(&id)
        .map(|s| {
            s.send(LoaderJob::Fetch { url, method, body, headers, reply: tx })
                .is_ok()
        })
        .unwrap_or(false);
...
fn op_do_call(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_do_call_impl(
    ...
    let name = (!name_value.is_null_or_undefined())
        .then(|| name_value.to_rust_string_lossy(scope));
    ...
    let headers = serde_json::from_str(&args.get(5).to_rust_string_lossy(scope))
        .unwrap_or_default();
    let (tx, rx) = tokio::sync::oneshot::channel();
    let (request_id, cancel, cancel_guard) = if cancellable {
        let request_id = next_do_request_id();
    ...
    let sent = DO_CALL_TX.get().map(|t| t.send(DoCallReq {
        request_id,
        cancel,
        scope: cell,
        name,
    ...
    let id = asyncrt::enqueue(async move {
        let _cancel_guard = cancel_guard;
        if !sent { return Err("no proxy channel".into()); }
        match rx.await {
...
fn op_do_call_cancel(
    ...
    if let Some(cancel) = do_call_cancels()
        .lock()
        .unwrap()
        .remove(&request_id)
    {
...
/// arguments and result are V8 structured-clone bytes.
fn op_rpc_call(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    let cell = args.get(0).to_rust_string_lossy(scope);
    let name_value = args.get(1);
    let name = (!name_value.is_null_or_undefined())
        .then(|| name_value.to_rust_string_lossy(scope));
    ...
    let (tx, rx) = tokio::sync::oneshot::channel();
    let sent = RPC_CALL_TX.get().map(|t| t.send(RpcCallReq {
        scope: cell, name, method, args, reply: tx,
    }).is_ok()).unwrap_or(false);
    let id = asyncrt::enqueue(async move {
        if !sent { return Err("no RPC channel".into()); }
        match rx.await {
            Ok(Ok(RpcData::V8(bytes))) => Ok(bytes),
            Ok(Ok(RpcData::Json(_))) => {
                Err("RPC answered JSON to a structured-clone call".into())
            }
            Ok(Err(e)) => Err(format!("{e}")),
...
fn op_fetch(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    ...
    let id = asyncrt::enqueue(async move {
        ...
        let fetch_timeout = std::env::var("CELLD_FETCH_TIMEOUT_S").ok()
            .and_then(|value| value.parse().ok()).filter(|seconds| *seconds > 0)
            .unwrap_or(120);
        ...
        if let Some(b) = body { rb = rb.body(b); }
        match rb.send().await {
            Ok(resp) => {
...
/// Move a host response source into a channel-owned pump. This stream lifetime
/// intentionally outlives the JS handler: the receiver returned in
/// HttpResponse owns it, and dropping the receiver stops the pump.
fn pump_http_stream(mut source: HttpStreamSource) -> HttpChunkReceiver {
...
fn http_stream_receiver(source: HttpStreamSource) -> HttpChunkReceiver {
    match source {
        HttpStreamSource::Receiver(receiver) => receiver,
        source => pump_http_stream(source),
    }
}
...
fn op_http_stream_read(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                       mut rv: v8::ReturnValue<v8::Value>) {
    ...
    let source = http_streams()
        ...
        .get_mut(&stream_id)
        .and_then(|stream| {
            stream.source.take().map(|source| {
                (source, stream.cancelled.subscribe())
            })
        });
    let id = asyncrt::enqueue(async move {
...
fn op_http_stream_cancel(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                         _rv: v8::ReturnValue<v8::Value>) {
    let stream_id = args.get(0).integer_value(scope).unwrap_or(0).max(0) as u64;
...
fn op_response_stream_create(
    ...
    writers.insert(stream_id, ResponseStreamWriter { created: now, writer, finished });
...
fn op_response_stream_closed(
    ...
    let id = asyncrt::enqueue(async move {
        ...
        Ok(String::from(if cancelled { "cancelled" } else { "finished" }))
    });
    rv.set(promise_for(scope, id));
}
...
fn op_response_stream_close(
    ...
    let stream = response_stream_writers()
        .lock()
        .unwrap()
        .remove(&stream_id);
    let id = asyncrt::enqueue(async move {
...
pub fn reqwest_response_stream(response: reqwest::Response) -> HttpChunkReceiver {
    pump_http_stream(HttpStreamSource::Response(response))
}

/// Timer op behind `setTimeout`: a promise resolving after `ms`.
fn op_timer(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_timer_alloc(scope: &mut v8::PinScope, _args: v8::FunctionCallbackArguments,
                  mut rv: v8::ReturnValue<v8::Value>) {
    let id = v8::Number::new(scope, NEXT_TIMER_ID.fetch_add(1, Ordering::Relaxed) as f64);
    rv.set(id.into());
}

fn op_timer_cancel(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                   _rv: v8::ReturnValue<v8::Value>) {
    let timer_id = args.get(0).integer_value(scope).unwrap_or(0).max(0) as u64;
...
fn op_do_id(
    ...
    match operation.as_str() {
        "name" => {
            ...
            rv.set(v8::String::new(scope, &durable_object_id_hex(&id)).unwrap().into());
            return;
        }
        "unique" => {
        ...
        "validate" => {
            ...
            rv.set(v8::String::new(scope, &input.to_ascii_lowercase()).unwrap().into());
            return;
        }
        _ => {
...
fn rsa_jwk_uint(value: &serde_json::Value, name: &str) -> Result<rsa::BigUint> {
    use base64::Engine;
    let encoded = value.get(name).and_then(|value| value.as_str())
...
fn crypto_operation(operation: &str, args: &serde_json::Value) -> Result<serde_json::Value> {
    match operation {
        "hmac-sign" => {
            ...
            let hash = args.get("hash").and_then(|value| value.as_str()).unwrap_or("SHA-256");
...
fn op_crypto_operation(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                       mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_webcrypto_random(
    ...
    let bytes = unsafe {
        std::slice::from_raw_parts_mut(
            store[offset..offset + len].as_ptr() as *mut u8,
            len,
        )
    };
...
fn op_webcrypto_digest(
    ...
    let Some(bytes) = view_bytes(args.get(1)) else { return };
...
fn op_webcrypto_hmac_sign(
    ...
    use hmac::Mac;
    let algorithm = args.get(0).to_rust_string_lossy(scope).to_ascii_uppercase();
    let Some(key) = view_bytes(args.get(1)) else { return };
    let Some(data) = view_bytes(args.get(2)) else { return };
    macro_rules! sign {
        ($digest:ty) => {{
            let Ok(mut mac) =
                <hmac::Hmac<$digest> as hmac::Mac>::new_from_slice(&key)
            else {
                return;
            };
            mac.update(&data);
...
fn pbkdf2_kdf<D>(password: &[u8], salt: &[u8], iterations: u32, keylen: usize)
    -> Vec<u8>
where
    D: hmac::digest::Digest + hmac::digest::core_api::BlockSizeUser,
{
    ...
    while out.len() < keylen {
        let mut u = hmac_once::<D>(
            password, &[salt, &block.to_be_bytes()]);
        let mut t = u.clone();
...
fn op_node_pbkdf2(
    ...
    let Some(password) = view_bytes(args.get(1)) else { return };
    let Some(salt) = view_bytes(args.get(2)) else { return };
    ...
    let out = dispatch_digest!(
        algorithm.as_str(), pbkdf2_kdf, (&password, &salt, iterations, keylen));
    if let Some(out) = out {
...
fn op_node_hkdf(
    ...
    let algorithm = args.get(0).to_rust_string_lossy(scope);
    let Some(ikm) = view_bytes(args.get(1)) else { return };
    let Some(salt) = view_bytes(args.get(2)) else { return };
    let Some(info) = view_bytes(args.get(3)) else { return };
    let length = args.get(4).uint32_value(scope).unwrap_or(0) as usize;
    let out = dispatch_digest!(
        algorithm.as_str(), hkdf_kdf, (&ikm, &salt, &info, length));
    if let Some(out) = out {
        webcrypto_return_bytes(scope, rv, &out);
    }
}
...
fn op_webcrypto_hmac_verify(
    ...
    let algorithm = args.get(0).to_rust_string_lossy(scope).to_ascii_uppercase();
    let Some(key) = view_bytes(args.get(1)) else { return };
    let Some(signature) = view_bytes(args.get(2)) else { return };
    let Some(data) = view_bytes(args.get(3)) else { return };
    macro_rules! verify {
        ($digest:ty) => {{
            let Ok(mut mac) =
                <hmac::Hmac<$digest> as hmac::Mac>::new_from_slice(&key)
...
fn op_webcrypto_aes_encrypt(
    ...
    use aes_gcm::aead::{Aead, KeyInit};
    let Some(key) = view_bytes(args.get(0)) else { return };
    let Some(iv) = view_bytes(args.get(1)) else { return };
    let Some(data) = view_bytes(args.get(2)) else { return };
    if iv.len() != 12 { return }
    let nonce = aes_gcm::Nonce::from_slice(&iv);
    let encrypted = match key.len() {
        16 => {
            let Ok(cipher) = aes_gcm::Aes128Gcm::new_from_slice(&key) else { return };
            cipher.encrypt(nonce, data.as_ref())
        }
        32 => {
            let Ok(cipher) = aes_gcm::Aes256Gcm::new_from_slice(&key) else { return };
            cipher.encrypt(nonce, data.as_ref())
        }
        _ => return,
    };
...
fn op_zlib(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
           mut rv: v8::ReturnValue<v8::Value>) {
    use std::io::{Read, Write};
    let mode = args.get(0).to_rust_string_lossy(scope);
    let bytes: Vec<u8> = serde_json::from_str(&args.get(1).to_rust_string_lossy(scope))
        .unwrap_or_default();
...
impl ZlibStream {
    fn new(format: &str, decompress: bool) -> Option<ZlibStream> {
        ...
        Some(match (format, decompress) {
            ("gzip", false) =>
                Self::GzipEncode(z::GzEncoder::new(sink(), level)),
            ("gzip", true) => Self::GzipDecode(z::GzDecoder::new(sink())),
            ("deflate", false) =>
                Self::ZlibEncode(z::ZlibEncoder::new(sink(), level)),
            ("deflate", true) => Self::ZlibDecode(z::ZlibDecoder::new(sink())),
            ("deflate-raw", false) =>
                Self::RawEncode(z::DeflateEncoder::new(sink(), level)),
            ("deflate-raw", true) =>
                Self::RawDecode(z::DeflateDecoder::new(sink())),
            _ => return None,
        })
    }

    fn decompress(&self) -> bool {
        matches!(self, Self::GzipDecode(_) | Self::ZlibDecode(_)
            | Self::RawDecode(_))
    }

    /// Workerd's canonical message, matched by its conformance suite.
    fn failure(&self) -> &'static str {
        if self.decompress() { "Decompression failed." }
...
fn zlib_stream_return(scope: &mut v8::PinScope,
                      rv: v8::ReturnValue<v8::Value>,
...
fn op_zlib_stream_new(scope: &mut v8::PinScope,
                      args: v8::FunctionCallbackArguments,
                      mut rv: v8::ReturnValue<v8::Value>) {
    let format = args.get(0).to_rust_string_lossy(scope);
    let decompress = args.get(1).boolean_value(scope);
    let stream = ZlibStream::new(&format, decompress)
        .expect("JS validated the compression format");
...
fn op_zlib_stream_push(scope: &mut v8::PinScope,
                       args: v8::FunctionCallbackArguments,
                       rv: v8::ReturnValue<v8::Value>) {
    ...
    let result = ZLIB_STREAMS.with(|streams| {
        ...
        if out.is_err() { streams.remove(&id); }
        out
    });
...
fn op_zlib_stream_end(scope: &mut v8::PinScope,
                      args: v8::FunctionCallbackArguments,
                      rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).integer_value(scope).unwrap_or(0) as u64;
    let result = ZLIB_STREAMS.with(|streams| {
...
fn op_zlib_stream_drop(scope: &mut v8::PinScope,
                       args: v8::FunctionCallbackArguments,
                       _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).integer_value(scope).unwrap_or(0) as u64;
...
fn op_ws_send(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
    let data = args.get(1).to_rust_string_lossy(scope);
    ws_emit(id, WsOut::Text(data));
}
fn op_ws_send_binary(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                     _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
    let data = view_bytes(args.get(1)).unwrap_or_default();
    ws_emit(id, WsOut::Binary(data));
}
fn op_ws_close(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
...
fn op_ws_alloc(scope: &mut v8::PinScope, _args: v8::FunctionCallbackArguments,
               mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_ws_upgrade(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                 mut rv: v8::ReturnValue<v8::Value>) {
    if actor_runtime_state(scope).egress == EgressPolicy::Deny {
        return loader_throw(
            scope,
    ...
    let sent = OUTBOUND_WS_TX.get().is_some_and(|sender| {
        sender.send(OutboundWsReq {
            scope: cell, id, url, protocols, pull,
            headers, want_response: true, reply: tx,
        }).is_ok()
    });
    let async_id = asyncrt::enqueue(async move {
        if !sent { return Err("no outbound WebSocket channel".into()); }
...
fn op_ws_next(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
              mut rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
    ...
    let async_id = asyncrt::enqueue(async move {
        ...
        Ok(queue.recv().await.unwrap_or_else(
            || WsPull::Close(1006, String::new(), false),
        ).encode())
    });
    rv.set(promise_for(scope, async_id));
}

fn op_ws_connect(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                 mut rv: v8::ReturnValue<v8::Value>) {
    if actor_runtime_state(scope).egress == EgressPolicy::Deny {
        return loader_throw(
            scope,
    ...
    let sent = OUTBOUND_WS_TX.get().is_some_and(|sender| {
        sender.send(OutboundWsReq {
            scope: cell, id, url, protocols, pull,
            headers: Vec::new(), want_response: false, reply: tx,
        }).is_ok()
    });
    let async_id = asyncrt::enqueue(async move {
        if !sent { return Err("no outbound WebSocket channel".into()); }
...
fn op_ws_accept(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
    let cell = args.get(1).to_rust_string_lossy(scope);
    let tags: Vec<String> = serde_json::from_str(&args.get(2).to_rust_string_lossy(scope))
        .unwrap_or_default();
    let replaced_regular_scope = {
        ...
        sockets.entry(id).and_modify(|meta| {
...
fn op_ws_accept_regular(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                        _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
    let cell = args.get(1).to_rust_string_lossy(scope);
    let inserted = {
...
fn op_ws_list(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
              mut rv: v8::ReturnValue<v8::Value>) {
    let cell = args.get(0).to_rust_string_lossy(scope);
    let tag = args.get(1);
    ...
    let rows = ws_registry().lock().unwrap().metadata.iter()
        .filter(|(_, meta)| meta.hibernatable && meta.scope == cell &&
...
fn op_ws_attachment_set(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                        _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).to_integer(scope).map(|n| n.value() as u64).unwrap_or(0);
...
fn op_storage_get(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    ...
    match storage::get_stored(&scope_s, &key) {
        Ok(Some(value)) => {
            if let Some((value, _)) =
                deserialize_stored(scope, value, args.get(2))
            {
                rv.set(value);
            }
        }
...
fn op_storage_get_many(
    ...
    match storage::get_many_stored(&cell, &keys) {
        Ok(entries) => {
            ...
            if let Some((map, tagged)) =
                storage_entries_map(scope, entries, sentinel)
            {
                rv.set(if tagged {
...
fn op_storage_put(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
...
fn op_process_exit(
    ...
    *state
        ...
        .expect("termination lock poisoned") = Some(ExecutionTermination {
            error: format!(
                "__CELLD_PROCESS_EXIT__:The Node.js process.exit({code}) API was called."
            ),
            actor_scope,
        });
    scope.terminate_execution();
...
fn op_log(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
    let msg: Vec<String> = (0..args.length())
        .map(|i| args.get(i).to_rust_string_lossy(scope)).collect();
...
fn op_sql_exec(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_storage_sync_list_next(
    ...
    match storage::sync_list_next(cursor) {
        Ok(Some((key, value))) => {
            let Some((decoded, _)) =
                deserialize_stored(scope, value, args.get(1))
            else {
                throw_storage_error(
                    scope,
                    "kv.list",
...
fn deserialize_stored<'s>(
    ...
    match value {
        storage::StoredValue::V8(bytes)
            if bytes.first() == Some(&STORED_STUB_TAG) =>
        {
            let tree = deserialize_storage_value(
                scope,
                storage::StoredValue::V8(bytes[1..].to_vec()),
            )?;
            Some((wrap_stored(scope, sentinel, tree), true))
        }
        value => deserialize_storage_value(scope, value)
...
fn op_sc_encode(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                mut rv: v8::ReturnValue<v8::Value>) {
...
fn op_sc_decode(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                mut rv: v8::ReturnValue<v8::Value>) {
    let Some(bytes) = view_bytes(args.get(0)) else {
        let message =
            v8::String::new(scope, "__sc_decode expects bytes").unwrap();
        let exception = v8::Exception::type_error(scope, message);
        scope.throw_exception(exception);
        return;
    };
...
fn op_storage_delete_all(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments,
                         _rv: v8::ReturnValue<v8::Value>) {
    let cell = args.get(0).to_rust_string_lossy(scope);
...
fn op_url_parse(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    let input = args.get(0).to_rust_string_lossy(scope);
    let base = args.get(1);
    let parsed = if base.is_undefined() {
        url::Url::parse(&input)
    } else {
        url::Url::options().base_url(url::Url::parse(&base.to_rust_string_lossy(scope)).ok().as_ref())
            .parse(&input)
    };
    let u = match parsed {
        Ok(u) => u,
...
fn op_urlpattern_parse(
    ...
    let base = args.get(1);
    let base = (!base.is_undefined())
        .then(|| base.to_rust_string_lossy(scope));
    let options = urlpattern::UrlPatternOptions {
        ignore_case: args.get(2).boolean_value(scope),
    };
    let pattern = quirks::process_construct_pattern_input(
        input, base.as_deref())
...
fn op_urlpattern_match_input(
    ...
    let out = serde_json::json!([
        m.protocol, m.username, m.password, m.hostname,
        m.port, m.pathname, m.search, m.hash,
    ]);
    rv.set(v8::String::new(scope, &out.to_string()).unwrap().into());
}

fn op_atob(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    use base64::Engine;
    let input = args.get(0).to_rust_string_lossy(scope);
...
fn op_util_preview_entries(
    ...
    let flag: v8::Local<v8::Value> =
        v8::Boolean::new(scope, is_key_value).into();
    rv.set(
        v8::Array::new_with_elements(scope, &[entries.into(), flag]).into(),
    );
}

fn op_alarm_set(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
    let s = args.get(0).to_rust_string_lossy(scope);
...
fn op_alarm_get(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    let s = args.get(0).to_rust_string_lossy(scope);
...
fn op_alarm_delete(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, _rv: v8::ReturnValue<v8::Value>) {
    let s = args.get(0).to_rust_string_lossy(scope);
...
fn op_btoa(scope: &mut v8::PinScope, args: v8::FunctionCallbackArguments, mut rv: v8::ReturnValue<v8::Value>) {
    use base64::Engine;
    let input = args.get(0).to_rust_string_lossy(scope);
...
fn op_text_decoder_label(scope: &mut v8::PinScope,
                         args: v8::FunctionCallbackArguments,
                         mut rv: v8::ReturnValue<v8::Value>) {
...
/// (TypeError).
fn op_text_decoder_decode(scope: &mut v8::PinScope,
                          args: v8::FunctionCallbackArguments,
                          mut rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).integer_value(scope).unwrap_or(0) as u64;
    let bytes = view_bytes(args.get(1)).unwrap_or_default();
    ...
    if !ok {
        let msg =
            v8::String::new(scope, "The encoded data was not valid.").unwrap();
        let exc = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exc);
        return;
    }
...
fn op_text_decoder_free(scope: &mut v8::PinScope,
                        args: v8::FunctionCallbackArguments,
                        _rv: v8::ReturnValue<v8::Value>) {
    let id = args.get(0).integer_value(scope).unwrap_or(0) as u64;
...
fn install_prelude(scope: &mut v8::PinScope) -> Result<()> {
    const PRELUDE: &[(&str, &str)] = &[
        ...
        ("url_search_params.js", include_str!("js/url_search_params.js")),
    ...
    for (name, src) in PRELUDE {
        ...
        let s = v8::Script::compile(scope, code, None)
            .ok_or_else(|| anyhow!("prelude compile {name}"))?;
        s.run(scope).ok_or_else(|| anyhow!("prelude run {name}"))?;
    }
    Ok(())
}

fn install_harness(scope: &mut v8::PinScope) -> Result<()> {
    let boot = include_str!("js/harness.js");
    let code = v8::String::new(scope, boot).unwrap();
    let s = v8::Script::compile(scope, code, None).ok_or_else(|| anyhow!("boot compile"))?;
    s.run(scope).ok_or_else(|| anyhow!("boot run"))?;
    let crypto = v8::String::new(scope, include_str!("js/crypto.js")).unwrap();
    let script = v8::Script::compile(scope, crypto, None)
        .ok_or_else(|| anyhow!("crypto prelude compile"))?;
    script.run(scope).ok_or_else(|| anyhow!("crypto prelude run"))?;
    Ok(())
}
...
mod smoke_tests {
    ...
    /// What one isolate costs to stand up — the number every addition to
    /// the prelude, the harness or the op table has to answer to. Ignored
    /// by default; run it as a controlled A/B (the same tree with and
    /// without a change), never as a before/after across a session:
    /// machine drift is larger than most changes being measured.
    ///
    ///     cargo test -p celld isolate_load_cost -- --ignored --nocapture
    #[test]
    #[ignore]
    fn isolate_load_cost() {
        init_engine_for_tests();
        asyncrt::init();
        let script =
        ...
        for _ in 0..5 {
            ...
            for _ in 0..60 {
                Worker::load(test_worker_options(script, "bench", &[], &[]), &[])
                    .expect("load");
            }
            rounds.push(start.elapsed().as_micros() / 60);
        }
        rounds.sort_unstable();
        println!("load: median={}us rounds={rounds:?}", rounds[2]);
    }

    #[test]
    fn raw_v8_durable_object_storage_smoke() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("PublicSmokeDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-public-js-smoke-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open smoke database");
        let script = r#"
          import { DurableObject } from "cloudflare:workers";
          export class PublicSmokeDO extends DurableObject {
            async ping(value) {
              await this.ctx.storage.put({});
              if (await this.ctx.storage.delete([]) !== 0)
                throw new Error("empty delete count");
              value = await this.ctx.blockConcurrencyWhile(() =>
                this.ctx.blockConcurrencyWhile(() => value)
              );
              const sql = this.ctx.storage.sql;
              sql.exec("CREATE TABLE IF NOT EXISTS smoke(value INTEGER)");
              sql.exec("INSERT INTO smoke VALUES (?)", value);
              return sql.exec("SELECT value FROM smoke").one().value;
            }
          }
        ...
        let mut worker = Worker::load(
            test_worker_options(
                script,
                "public-smoke-script",
                &["PublicSmokeDO".to_string()],
                &[("NS".to_string(), "PublicSmokeDO".to_string())],
            ),
            std::slice::from_ref(&scope),
        )
        .expect("load smoke worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        assert_eq!(
            worker
                .dispatch_rpc(&scope, "ping", "[42]", &receiver)
                .expect("dispatch smoke RPC"),
            "42",
        );
    ...
    #[test]
    fn raw_v8_durable_object_routing_error_surface() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("RoutingErrorDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-public-routing-error-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open routing-error database");
        let script = r#"
          import { DurableObject } from "cloudflare:workers";
          export class RoutingErrorDO extends DurableObject {
            probe() {
              const marker = "__CELLD_DO_ROUTING_ERROR__:";
              const error = __durableObjectRoutingError(new Error(
                marker + JSON.stringify({
                  scope: "RoutingErrorDO:abc",
                  owner: "10.0.0.1:8080",
                }),
              ));
              return {
                instance: error instanceof DurableObjectRoutingError,
                name: error.name,
                code: error.code,
        ...
        let mut worker = Worker::load(
            test_worker_options(
                script,
                "routing-error-script",
                &["RoutingErrorDO".to_string()],
                &[("NS".to_string(), "RoutingErrorDO".to_string())],
            ),
            std::slice::from_ref(&scope),
        )
        .expect("load routing-error worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let got: serde_json::Value = serde_json::from_str(
            &worker
                .dispatch_rpc(&scope, "probe", "[]", &receiver)
                .expect("dispatch routing-error probe"),
        )
        .expect("routing-error probe is JSON");
        assert_eq!(got["instance"], true);
        assert_eq!(got["name"], "DurableObjectRoutingError");
        assert_eq!(got["code"], "owner_unreachable");
        assert_eq!(got["retryable"], true);
        assert_eq!(
            got["message"],
            "The Durable Object owner is currently unreachable",
        );
        assert_eq!(got["scope"], "RoutingErrorDO:abc");
        assert_eq!(got["owner"], "10.0.0.1:8080");
    ...
    /// A service-binding caller that disconnects must reach the *target's*
    /// `request.signal`, so a hanging handler stops instead of running to
    /// completion. Proven by timing: the handler would otherwise sit on a five
    /// second timer.
    #[test]
    fn service_target_request_aborts_mid_flight() {
        init_engine_for_tests();
        asyncrt::init();
        let script = r#"
          globalThis.__abortSeen = false;
          export default {
            async fetch(request) {
              await new Promise((resolve) => {
                request.signal.addEventListener("abort", () => {
                  globalThis.__abortSeen = true;
                  resolve();
                }, { once: true });
                setTimeout(resolve, 5000);
              });
              return new Response(request.signal.aborted ? "aborted" : "timeout");
            },
          };
        "#;
        let mut worker = Worker::load(
            test_worker_options(script, "abort-script", &[], &[]),
            &[],
        )
        .expect("load abort worker");
        ...
        worker
            .fetch_and_reply_id(
                "https://internal/hang", "GET", b"", &[], Some(request_id), reply,
            )
            .expect("dispatch hanging fetch");
        let outcome = result.blocking_recv().expect("reply channel");
        let elapsed = started.elapsed();
        ...
        // Well under the handler's own 5s timer: the abort, not the timeout,
        // ended it.
        assert!(
            elapsed < std::time::Duration::from_secs(3),
            "abort did not reach the target: took {elapsed:?}; {outcome_description}",
        );
        // The run loop reports the cancellation rather than a response.
        assert!(outcome.is_err(), "cancelled request should not reply normally");
        let context_g = worker.context.clone();
        v8::scope!(let hs, &mut worker.isolate);
        let context = v8::Local::new(hs, context_g);
        let scope = &mut v8::ContextScope::new(hs, context);
        let key = v8::String::new(scope, "__abortSeen").unwrap();
        let seen = context
            .global(scope)
            .get(scope, key.into())
            .expect("abort observation");
        assert!(
            seen.boolean_value(scope),
            "the target request.signal did not fire; took {elapsed:?}; {outcome_description}",
        );
    }

    /// `entrypoint = "Name"` turns the binding into an RPC stub over that
    /// class. Real arguments and return values, not an HTTP round trip.
    #[test]
    fn service_binding_entrypoint_rpc() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("EntryDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-entry-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open entrypoint database");
        let script = r#"
          import { DurableObject, WorkerEntrypoint } from "cloudflare:workers";
          export class Greeter extends WorkerEntrypoint {
            fetch() { return new Response("from Greeter"); }
            greet(name, punctuation) {
              return { hello: name + punctuation, hasEnv: !!this.env };
            }
            async slow(value) { return value * 2; }
            notAFunction = 5;
          }
          export class EntryDO extends DurableObject {
            async probe() {
              const out = {};
              // fetch() on an entrypoint-bound binding must reach the
              // entrypoint's own handler, not the module default export.
              out.entrypointFetch =
                await (await this.env.AUTH.fetch("https://internal/")).text();
              out.greet = await this.env.AUTH.greet("world", "!");
              out.slow = await this.env.AUTH.slow(21);
              // fetch() stays available alongside the RPC methods.
              out.hasFetch = typeof this.env.AUTH.fetch === "function";
              // A stub must never look like a promise.
              out.notThenable = this.env.AUTH.then === undefined;
              try {
                await this.env.AUTH.missing();
                out.missing = "resolved";
              } catch (error) {
                out.missing = String(error && error.message);
              }
              return out;
            }
          }
        ...
        let config = std::sync::Arc::new(
            WorkerConfig::new(test_worker_options(
                script,
                "entry-script",
                &["EntryDO".to_string()],
                &[("NS".to_string(), "EntryDO".to_string())],
            ))
            .with_services(vec![(
                "AUTH".to_string(),
        ...
        let mut worker =
            Worker::load_config(config, std::slice::from_ref(&scope))
                .expect("load entrypoint worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let got: serde_json::Value = serde_json::from_str(
            &worker
                .dispatch_rpc(&scope, "probe", "[]", &receiver)
        ...
        assert_eq!(got["entrypointFetch"], "from Greeter");
        assert_eq!(got["greet"]["hello"], "world!");
        assert_eq!(got["greet"]["hasEnv"], true);
        assert_eq!(got["slow"], 42);
        assert_eq!(got["hasFetch"], true);
        assert_eq!(got["notThenable"], true);
        // Workerd's canonical message for an unknown entrypoint RPC method.
        assert_eq!(
            got["missing"],
            "The RPC receiver does not implement the method \"missing\"."
        );
    ...
    /// Lazy globals arrive fully formed, and runtime plumbing stays out of
    /// the way of a bundle that walks `globalThis`.
    #[test]
    fn identity_transform_stream_and_hidden_internals() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("LazyDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-lazy-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open lazy-global database");
        let script = r#"
          import { DurableObject } from "cloudflare:workers";
          export class LazyDO extends DurableObject {
            async probe() {
              const out = {};
              // A subclass of TransformStream, so the base is not an
              // instance of it.
              out.subclass = new IdentityTransformStream() instanceof
                TransformStream;
              out.notBase = !(new TransformStream() instanceof
                IdentityTransformStream);
              const { readable, writable } = new IdentityTransformStream();
              const writer = writable.getWriter();
              const reader = readable.getReader();
              const [, first] = await Promise.all([
                writer.write(new TextEncoder().encode("hi")),
                reader.read(),
              ]);
              out.passthrough = new TextDecoder().decode(first.value);
              // The writable side is bytes only.
              const bytesOnly = new IdentityTransformStream();
              bytesOnly.readable.getReader().read().catch(() => {});
              out.rejectsStrings = await bytesOnly.writable.getWriter()
                .write("nope").then(() => "resolved", (e) => e.name);
              // A FixedLengthStream that never sees its bytes errors on
              // close rather than closing short.
              const fixed = new FixedLengthStream(8);
              fixed.readable.getReader().read().catch(() => {});
              const short = fixed.writable.getWriter();
              await short.write(new Uint8Array(4));
              out.shortClose = await short.close().then(
                () => "resolved", (e) => e.message);
              // Runtime internals must not show up in a `for…in` walk of
              // the globals: one of them used to abort the actor when
              // constructed.
              out.hiddenOps = Object.keys(globalThis)
                .filter((k) => k.startsWith("__cell") ||
                               k.startsWith("__actor")).length;
              return out;
            }
          }
          export default { fetch() { return new Response("ok"); } };
        ...
        let mut worker = Worker::load(
            test_worker_options(
                ...
                "lazy-script",
                &["LazyDO".to_string()],
                &[("NS".to_string(), "LazyDO".to_string())],
            ),
            std::slice::from_ref(&scope),
        )
        .expect("load lazy-global worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let got: serde_json::Value = serde_json::from_str(
            &worker
                .dispatch_rpc(&scope, "probe", "[]", &receiver)
        ...
        .expect("probe result is JSON");
        assert_eq!(got["subclass"], true);
        assert_eq!(got["notBase"], true);
        assert_eq!(got["passthrough"], "hi");
        assert_eq!(got["rejectsStrings"], "TypeError");
        assert_eq!(
            got["shortClose"],
            "FixedLengthStream did not see all expected bytes before close().",
        );
        assert_eq!(got["hiddenOps"], 0);
    ...
    /// A binding targeting this same script runs the handler in this isolate
    /// rather than crossing to a pool thread. Proven by module state the
    /// caller can observe: a pool thread would be a different isolate and
    /// would not see the mutation.
    #[test]
    fn self_service_binding_runs_in_isolate() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("SelfSvcDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-self-svc-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open self service binding database");
        let script = r#"
          import { DurableObject } from "cloudflare:workers";
          let touched = 0;
          export class SelfSvcDO extends DurableObject {
            async probe() {
              const before = touched;
              const res = await this.env.SELF.fetch("https://internal/ping");
              return {
                body: await res.text(),
                status: res.status,
                // Same isolate ⇒ the handler's mutation is visible here.
                sameIsolate: touched === before + 1,
              };
            }
            async probeRecursion() {
              try {
                await this.env.SELF.fetch("https://internal/recurse");
                return "resolved";
              } catch (error) {
          ...
          export default {
            async fetch(request) {
              ...
              if (url.pathname === "/recurse")
                return await __cell.env.SELF.fetch(request);
        ...
        let config = std::sync::Arc::new(
            WorkerConfig::new(test_worker_options(
                ...
                "self-svc-script",
                &["SelfSvcDO".to_string()],
                &[("NS".to_string(), "SelfSvcDO".to_string())],
            ))
            .with_services(vec![(
                "SELF".to_string(),
                "self-svc-script".to_string(),
                None,
            )]),
        );
        let mut worker =
            Worker::load_config(config, std::slice::from_ref(&scope))
                .expect("load self service binding worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let got: serde_json::Value = serde_json::from_str(
            &worker
                .dispatch_rpc(&scope, "probe", "[]", &receiver)
        ...
        assert_eq!(got["sameIsolate"], true);

        // Unbounded self-recursion must be refused rather than exhausting the
        // stack.
        let recursion: serde_json::Value = serde_json::from_str(
            &worker
                .dispatch_rpc(&scope, "probeRecursion", "[]", &receiver)
                .expect("dispatch recursion probe"),
        )
        .expect("recursion result is JSON");
        assert_eq!(
            recursion,
    ...
    /// A service binding surfaces on `env` as a Fetcher. The dispatch itself
    /// needs the router in `main`, so this asserts the binding's shape and
    /// that an unrouted call rejects rather than silently resolving — the
    /// failure mode that would make a missing target look like success.
    #[test]
    fn service_binding_surfaces_on_env() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("SvcDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-svc-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open service binding database");
        let script = r#"
          import { DurableObject } from "cloudflare:workers";
          export class SvcDO extends DurableObject {
            async probe() {
              const binding = this.env.AUTH;
              const shape = {
                present: binding !== undefined,
                hasFetch: typeof binding.fetch === "function",
              };
              try {
                await binding.fetch("https://internal/verify");
                shape.call = "resolved";
              } catch (error) {
                shape.call = "rejected";
              }
              // An already-aborted signal must surface the signal's reason,
              // not a generic dispatch failure, and must not hang.
              const controller = new AbortController();
              controller.abort(new Error("gone"));
              try {
                await binding.fetch(new Request("https://internal/verify", {
                  signal: controller.signal,
                }));
                shape.aborted = "resolved";
              } catch (error) {
                shape.aborted = String(error && error.message);
              }
              return shape;
            }
          }
        ...
        let mut worker =
            Worker::load_config(config, std::slice::from_ref(&scope))
                .expect("load service binding worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let raw = worker
            .dispatch_rpc(&scope, "probe", "[]", &receiver)
            .expect("dispatch probe");
        let got: serde_json::Value =
            serde_json::from_str(&raw).expect("probe result is JSON");
        assert_eq!(got["present"], true);
        assert_eq!(got["hasFetch"], true);
        // No router is installed in this test, so the call must reject.
        assert_eq!(got["call"], "rejected");
        // The caller's abort reason wins over the dispatch failure.
        assert_eq!(got["aborted"], "gone");
    ...
    /// `node:assert` must actually throw. A pass-through proxy would make
    /// every assertion succeed silently, and a test suite importing it would
    /// pass vacuously.
    /// Each case records "pass" (no throw), "assert" (AssertionError), or
    /// "error" (some other throw).
    #[test]
    fn node_assert_enforces_assertions() {
        init_engine_for_tests();
        asyncrt::init();
        let sequence = NEXT_DATABASE.fetch_add(1, Ordering::Relaxed);
        let scope = format!("AssertDO:{sequence}");
        let path = std::env::temp_dir().join(format!(
            "cells-assert-{}-{sequence}.sqlite",
            std::process::id(),
        ));
        storage::open(&scope, path.to_str().expect("temporary path is UTF-8"))
            .expect("open assert database");
        let script = r#"
          import assert, {
            strictEqual, notStrictEqual, deepStrictEqual, ok, throws,
            rejects, match, fail, AssertionError,
          } from "node:assert";
          import { DurableObject } from "cloudflare:workers";
          const check = async (f) => {
            try { await f(); return "pass"; } catch (e) {
              return e instanceof AssertionError || e.name === "AssertionError"
                ? "assert" : "error";
            }
          };
          export class AssertDO extends DurableObject {
            async run() {
              const r = {};
              // Must PASS.
              r.okTrue = await check(() => ok(1));
              r.strictSame = await check(() => strictEqual("a", "a"));
              r.nanIsNan = await check(() => strictEqual(NaN, NaN));
              r.notStrict = await check(() => notStrictEqual(1, 2));
              r.deepNested = await check(() =>
                deepStrictEqual({ a: [1, { b: 2 }] }, { a: [1, { b: 2 }] }));
              r.deepMap = await check(() =>
                deepStrictEqual(new Map([["a", 1]]), new Map([["a", 1]])));
              r.deepSetUnordered = await check(() =>
                deepStrictEqual(new Set([1, 2]), new Set([2, 1])));
              r.deepDate = await check(() =>
                deepStrictEqual(new Date(5), new Date(5)));
              r.deepBytes = await check(() =>
                deepStrictEqual(new Uint8Array([1, 2]), new Uint8Array([1, 2])));
              r.deepRegExp = await check(() => deepStrictEqual(/x/g, /x/g));
              r.throwsCtor = await check(() =>
                throws(() => { throw new TypeError("boom"); }, TypeError));
              r.throwsShape = await check(() =>
                throws(() => { throw new Error("boom"); }, { message: "boom" }));
              r.rejectsThrown = await check(() =>
                rejects(async () => { throw new Error("x"); }));
              r.matchHit = await check(() => match("hello", /ell/));
              r.assertTrue = await check(() => assert(true));
              // Must FAIL with AssertionError.
              r.okFalse = await check(() => ok(0));
              r.strictDiff = await check(() => strictEqual(1, 2));
              r.signedZero = await check(() => strictEqual(0, -0));
              r.strictTypes = await check(() => strictEqual(1, "1"));
              r.deepValueDiff = await check(() =>
                deepStrictEqual({ a: 1 }, { a: 2 }));
              r.deepTypeCoerce = await check(() =>
                deepStrictEqual({ a: 1 }, { a: "1" }));
              r.deepExtraKey = await check(() =>
                deepStrictEqual({ a: 1 }, { a: 1, b: 2 }));
              r.deepMapDiff = await check(() =>
                deepStrictEqual(new Map([["a", 1]]), new Map([["a", 2]])));
              r.deepBytesDiff = await check(() =>
                deepStrictEqual(new Uint8Array([1, 2]), new Uint8Array([1, 3])));
              r.deepDateDiff = await check(() =>
                deepStrictEqual(new Date(5), new Date(6)));
              r.throwsNothing = await check(() => throws(() => {}));
              r.rejectsResolved = await check(() => rejects(async () => 1));
              r.matchMiss = await check(() => match("hello", /zzz/));
              r.failAlways = await check(() => fail("nope"));
              r.assertFalse = await check(() => assert(false));
              // A non-assertion throw must surface unchanged, and a `throws`
              // whose matcher does not match must rethrow the original.
              r.throwsWrongCtor = await check(() =>
                throws(() => { throw new TypeError("x"); }, RangeError));
              // Circular structures must terminate rather than recurse away.
              const a = { v: 1 }; a.self = a;
              const b = { v: 1 }; b.self = b;
              r.deepCircular = await check(() => deepStrictEqual(a, b));
              return r;
            }
          }
          export default { fetch() { return new Response("ok"); } };
        ...
        let mut worker = Worker::load(
            test_worker_options(
                ...
                "assert-script",
                &["AssertDO".to_string()],
                &[("NS".to_string(), "AssertDO".to_string())],
            ),
            std::slice::from_ref(&scope),
        )
        .expect("load assert worker");
        let (_sender, receiver) = std::sync::mpsc::channel();
        let raw = worker
            .dispatch_rpc(&scope, "run", "[]", &receiver)
            .expect("dispatch assert RPC");
        let got: serde_json::Value =
            serde_json::from_str(&raw).expect("assert result is JSON");

        let passes = [
            "okTrue", "strictSame", "nanIsNan", "notStrict", "deepNested",
            "deepMap", "deepSetUnordered", "deepDate", "deepBytes",
            "deepRegExp", "throwsCtor", "throwsShape", "rejectsThrown",
            "matchHit", "assertTrue", "deepCircular",
        ];
        let failures = [
            "okFalse", "strictDiff", "signedZero", "strictTypes",
            "deepValueDiff", "deepTypeCoerce", "deepExtraKey", "deepMapDiff",
            "deepBytesDiff", "deepDateDiff", "throwsNothing",
            "rejectsResolved", "matchMiss", "failAlways", "assertFalse",
        ];
        for key in passes {
            assert_eq!(got[key], "pass", "{key} should not have thrown");
        }
        for key in failures {
            assert_eq!(got[key], "assert", "{key} should be an AssertionError");
        }
        // The original TypeError propagates; it is not swallowed or reshaped.
        assert_eq!(got["throwsWrongCtor"], "error");
...
fn register_class(scope: &mut v8::PinScope, name: &str, cls: v8::Local<v8::Value>) -> Result<()> {
    ...
    let cell = global.get(scope, ck.into()).unwrap().to_object(scope).unwrap();
    let clk = v8::String::new(scope, "classes").unwrap();
    let classes = cell.get(scope, clk.into()).unwrap().to_object(scope).unwrap();
...
fn populate_cf_exports(
    ...
    let make_ns: v8::Local<v8::Function> = cell
        .get(scope, v8::String::new(scope, "makeNamespace").unwrap().into())
        .and_then(|value| value.try_into().ok())
        .ok_or_else(|| anyhow!("missing __cell.makeNamespace"))?;
    let do_set: std::collections::HashSet<&str> =
        do_classes.iter().map(String::as_str).collect();
    let names = ns
    ...
    for index in 0..names.length() {
        ...
        let export_value = if do_set.contains(name.as_str()) {
            ...
            ns.get(scope, name_value).ok_or_else(|| anyhow!("export {name}"))?
        };
...
fn inject_namespace_keys(
    ...
    for class_name in do_classes {
        ...
        let namespace_key = format!(
            "cells:v1:{}:{script_name}:{class_name}",
            script_name.len(),
        );
...
fn build_env(scope: &mut v8::PinScope, config: &WorkerConfig) -> Result<()> {
    ...
    let src = {
        ...
        for (bname, cname) in bindings {
            lines.push_str(&format!("e[{:?}] = __cell.makeNamespace({:?});\n", bname, cname));
        }
...
fn register_entrypoints(
    ...
    let Some(names) = ns.get_own_property_names(scope, Default::default())
    else {
        return Ok(());
    };
    ...
    for index in 0..names.length() {
        ...
        let Some(value) = ns.get(scope, name) else { continue };
        ...
        while let Some(current) = proto {
            ...
            proto = current.to_object(scope).and_then(|o| o.get_prototype(scope));
        }
    }
    Ok(())
}
...
fn inject_storage_compatibility(
    scope: &mut v8::PinScope,
    compat: Compat,
) -> Result<()> {
    let source = format!(
...
fn begin_event_context<'s>(
    scope: &mut v8::PinScope<'s, '_>,
) -> Result<v8::Local<'s, v8::Value>> {
    let global = scope.get_current_context().global(scope);
    let key = v8::String::new(scope, "__beginEvent").unwrap();
    let function: v8::Local<v8::Function> = global.get(scope, key.into())
        .ok_or_else(|| anyhow!("no __beginEvent"))?
        .try_into().map_err(|_| anyhow!("__beginEvent is not a function"))?;
    let recv = v8::undefined(scope).into();
...
fn end_event_context<'s>(
    ...
    let function: v8::Local<v8::Function> = global.get(scope, key.into())
        ...
        .try_into().map_err(|_| anyhow!("__endEvent is not a function"))?;
    let recv = v8::undefined(scope).into();
    let value = function.call(scope, recv, &[])
        .ok_or_else(|| anyhow!("__endEvent threw"))?;
    if value.is_null_or_undefined() {
        Ok(None)
    } else {
        Ok(Some(value.try_into()
            .map_err(|_| anyhow!("__endEvent did not return a promise"))?))
    }
}

// ---- module compilation + request/response marshalling ----

fn compile_module<'s>(scope: &mut v8::PinScope<'s, '_>, name: &str, src: &str) -> Option<v8::Local<'s, v8::Module>> {
...
fn is_external(spec: &str) -> bool {
    const BARE: &[&str] = &["path", "os", "fs", "fs/promises", "crypto", "stream",
        "util", "util/types", "assert", "module", "process", "tty", "url", "v8",
        "constants", "buffer", "events", "zlib", "child_process", "http", "https",
        "http2", "net", "tls", "dns", "readline", "string_decoder", "querystring",
        "async_hooks", "perf_hooks", "worker_threads", "vm", "diagnostics_channel",
        "timers", "inspector", "dgram", "cluster", "punycode", "sqlite"];
    if spec.starts_with("node:") || spec.starts_with("cloudflare:") { return true; }
    // bare builtin, or a builtin submodule like `stream/promises`
    BARE.contains(&spec) || BARE.contains(&spec.split('/').next().unwrap_or(""))
}

fn scan_external_imports(src: &str)
    -> std::collections::BTreeMap<String, std::collections::BTreeSet<String>>
{
    let re = regex::Regex::new(
        r#"import\s+([^;]*?)\s*from\s*["']([^"']+)["']"#).unwrap();
    let braces = regex::Regex::new(r"\{([^}]*)\}").unwrap();
    let mut map: std::collections::BTreeMap<String, std::collections::BTreeSet<String>> =
        Default::default();
    for cap in re.captures_iter(src) {
        if !is_external(&cap[2]) { continue; }
...
fn lazy_global_getter(
    ...
    for other in group.names {
        ...
        if name.strict_equals(key.into()) { continue; }
...
fn stub_source(spec: &str, names: &std::collections::BTreeSet<String>) -> String {
    ...
    for n in names {
        ...
        } else if cf && matches!(n.as_str(),
                "DurableObject" | "RpcTarget" | "WorkerEntrypoint" | "env"
                    | "exports") {
...
fn register_stubs(scope: &mut v8::PinScope, src: &str, text: &[(String, String)]) {
    ...
    let reg = |spec: String, source: String, scope: &mut v8::PinScope| {
        ...
        } else { tracing::warn!(%spec, "module stub failed to compile"); }
    };
    ...
    for (spec, content) in text {
        let s = format!("export default {};", serde_json::to_string(content).unwrap());
        reg(spec.clone(), s, scope);
    }
    tracing::debug!(mods = MODREG.with(|r| r.borrow().len()), "registered import stubs + text modules");
}
    ...
    for (_name, source) in modules {
        for (spec, names) in scan_external_imports(source) {
            ...
            if let Some(m) = compile_module(scope, &spec, &s) {
                MODREG.with(|r| {
                    r.borrow_mut().insert(spec, v8::Global::new(scope, m))
                });
            }
        }
    }
...
fn resolve_external<'s>(
    ...
    if m.is_none() { tracing::warn!(%spec, "resolve: no stub for specifier"); }
    m
}
...
fn builtin_source(spec: &str) -> Option<(String, String)> {
    ...
    is_external(spec)
        .then(|| (String::new(), "globalThis.__nodeStub".to_string()))
}
...
fn op_builtin_module(
    ...
    let Some((setup, expr)) = builtin_source(&spec) else { return };
...
fn dynamic_namespace<'s>(
    ...
    let cached = MODREG
        .with(|r| r.borrow().get(&key).map(|g| v8::Local::new(scope, g)));
    ...
    MODREG.with(|r| {
        r.borrow_mut().insert(key, v8::Global::new(scope, module))
    });
...
fn host_import_module_dynamically<'s>(
    ...
    match dynamic_namespace(tc, &spec) {
        Ok(namespace) => { resolver.resolve(tc, namespace); }
        Err(error) => {
            let caught = tc.exception();
            tc.reset();
            let exception = caught.unwrap_or_else(|| {
                let message =
                    v8::String::new(tc, &error.to_string()).unwrap();
...
fn make_incoming_request<'s>(
    ...
    let headers = v8::String::new(
        tc, &serde_json::to_string(headers).unwrap_or_else(|_| "[]".into())).unwrap();
    let recv = v8::undefined(tc).into();
...
fn make_request<'s>(tc: &mut v8::PinScope<'s, '_>, url: &str, method: &str, body: &[u8],
                    headers: &[(String, String)])
    -> Result<v8::Local<'s, v8::Value>>
{
...
fn read_response(scope: &mut v8::PinScope, ret: v8::Local<v8::Value>,
                 cell_rx: Option<&std::sync::mpsc::Receiver<CellJob>>) -> Result<HttpResponse> {
    let ctx = scope.get_current_context();
    let global = ctx.global(scope);
    let key = v8::String::new(scope, "__readResponse").unwrap();
    let f: v8::Local<v8::Function> = global.get(scope, key.into())
        .ok_or_else(|| anyhow!("no __readResponse"))?.try_into().map_err(|_| anyhow!("not fn"))?;
    let recv = v8::undefined(scope).into();
    let out = f.call(scope, recv, &[ret]).ok_or_else(|| anyhow!("readResponse threw"))?;
    ...
    let status = out.get(scope, sk.into()).and_then(|v| v.uint32_value(scope)).unwrap_or(200) as u16;
    ...
    let body = out.get(scope, bk.into())
        ...
        .unwrap_or_default();
    let stream_id = out.get(scope, tk.into())
        .and_then(|value| value.integer_value(scope)).unwrap_or(0).max(0) as u64;
    let stream = if stream_id == 0 {
        ...
        http_streams()
            ...
            .remove(&stream_id)
            .and_then(|stream| stream.source)
            .map(http_stream_receiver)
    };
    if stream_id != 0 && stream.is_none() {
        return Err(anyhow!("response stream {stream_id} is no longer available"));
    }
    let headers = out.get(scope, hk.into())
        .and_then(|v| serde_json::from_str(&v.to_rust_string_lossy(scope)).ok())
        .unwrap_or_default();
    let ws_json = out.get(scope, wk.into())
        .map(|v| v.to_rust_string_lossy(scope)).unwrap_or_else(|| "null".into());
    let ws = serde_json::from_str(&ws_json).ok();
    if status == 101 {
        tracing::info!(%ws_json, has_target = ws.is_some(), "JS WebSocket response");
    }
    Ok(HttpResponse { status, body, stream, headers, ws })
}
