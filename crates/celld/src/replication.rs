...
pub async fn epoch_replicated(
    ...
    match c.list_objects_v2().bucket(bucket).prefix(prefix).max_keys(1)
        .send().await {
...
pub async fn restore_snapshot(
    ...
    let snapshot = RestoredSnapshot { epoch, path, directory };
...
impl NodeRepl {
    ...
    pub fn start(
        ...
        let mut socket = std::env::temp_dir().join(format!(
            "celld-ls-{}.sock",
            std::process::id()
        ));
...
"#,
    ...
    pub async fn sync_wait(
        ...
        match tokio::time::timeout(timeout, io).await {
            Ok(Ok(response)) if response.starts_with("HTTP/1.1 200") => SyncWait::Durable,
            Ok(Ok(response)) => {
    ...
    /// db.
    pub async fn activate(&self, options: ActivationOptions<'_>) -> anyhow::Result<ActivationResult> {
        ...
        let path = tokio::task::spawn_blocking(move || -> anyhow::Result<PathBuf> {
            ...
            if let Some(hibernated) = local_hibernated {
                ...
                crate::advisory_activity().record_restore();
                info!(
                    cell,
                    epoch,
    ...
    pub fn prune_local_cache(&self, max_bytes: u64) -> (usize, usize, u64) {
        ...
        let mut stack = vec![self.watch.clone()];
            ...
            for item in read.flatten() {
                ...
                if meta.is_dir() {
                    ...
                    let last_used_ms = meta
                        ...
                        .unwrap_or(0);
                    entries.push(CacheEntry { last_used_ms, bytes: meta.len() });
                    paths.push(path);
                }
            }
        }
    ...
    pub async fn hibernate(&self, cell: &str, epoch: u64, preserve_local: bool) {
        ...
        if preserve_local {
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                sqlite_snapshot(&snapshot_db, &cache_tmp).and_then(|()| {
        ...
        info!(cell, epoch, local_cache, "hibernated (replication released)");
    }
}
...
fn sqlite_snapshot(source: &std::path::Path, destination: &std::path::Path) -> anyhow::Result<()> {
    {
        let source =
...
fn apply_litestream_memory_limit(command: &mut Command) {
    command.env(
        "GOMEMLIMIT",
        std::env::var_os("GOMEMLIMIT")
            .unwrap_or_else(|| DEFAULT_LITESTREAM_GOMEMLIMIT.into()),
    );
}
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!(
            "cells-replication-{name}-{}-{sequence}",
            std::process::id()
        ));
    ...
    fn create_db(watch: &std::path::Path, cell: &str, epoch: u64, value: i64) -> PathBuf {
        let path = watch
            .join(cell)
            .join("ltx")
            .join(format!("e{epoch}"))
        ...
        connection
            .execute("CREATE TABLE state (value INTEGER NOT NULL)", [])
            .unwrap();
        connection
            .execute("INSERT INTO state VALUES (?)", [value])
            .unwrap();
        path
    }

    fn lock_runtime(watch: &std::path::Path) -> File {
    ...
    #[test]
    fn newest_local_epoch_is_strictly_before_activation() {
        let root = temp_root("epochs");
        let watch = root.join("node");
        create_db(&watch, "cell", 1, 1);
        create_db(&watch, "cell", 3, 3);
        create_db(&watch, "cell", 5, 5);
        std::fs::create_dir_all(watch.join("cell/ltx/not-an-epoch")).unwrap();

        assert_eq!(newest_epoch_in(&watch, "cell", 5).unwrap().0, 3);
        assert_eq!(newest_epoch_in(&watch, "cell", 6).unwrap().0, 5);
        assert!(newest_epoch_in(&watch, "cell", 1).is_none());
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn local_recovery_ignores_a_live_runtime_directory() {
        let root = temp_root("locks");
        let current = root.join("current");
        let dead = root.join("dead");
        let live = root.join("live");
        create_db(&current, "cell", 1, 1);
        create_db(&dead, "cell", 2, 2);
        create_db(&live, "cell", 3, 3);
        let current_lock = lock_runtime(&current);
        let dead_lock = lock_runtime(&dead);
        dead_lock.unlock().unwrap();
        drop(dead_lock);
        let live_lock = lock_runtime(&live);
        let repl = NodeRepl {
            watch: current,
            socket: root.join("no.sock"),
            sync_unsupported_warned: AtomicBool::new(false),
            child: Mutex::new(None),
            supervisor_stdin: None,
            _runtime_lock: current_lock,
        };

        let recovery = repl.local_recovery("cell", 4).unwrap();
        assert_eq!(recovery.epoch, 2);
        assert!(recovery.path.starts_with(&dead));
        drop(recovery);
        drop(repl);
        drop(live_lock);
        std::fs::remove_dir_all(root).unwrap();
    ...
    #[test]
    fn sqlite_snapshot_includes_committed_wal_state() {
        ...
        connection
            ...
            .unwrap();
        connection
            .execute("INSERT INTO state VALUES (42)", [])
            .unwrap();

        sqlite_snapshot(&source, &destination).unwrap();
