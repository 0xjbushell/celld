...
pub fn install_signer() -> anyhow::Result<String> {
    let signer = PeerProbeSigner::generate();
...
pub async fn probe(
    ...
    if !status.is_success() {
        // Distinguish an authentication rejection from an intermediary
        // (a proxy 502 is not the node speaking).
        return Err(if status.as_u16() == 401 || status.as_u16() == 403 {
            anyhow!("node {} rejected the signed probe (HTTP {status})", node.node)
        } else {
            anyhow!(
                "node {} probe failed with HTTP {status}; an intermediary \
                 may have answered instead of the node",
                node.node
            )
        });
    }
...
fn verify(
    expected: &crate::ownership::NodeRec,
    challenge: &str,
...
#[cfg(test)]
mod tests {
    use super::{verify, PeerProbeSigner};

    #[test]
    fn signed_probe_binds_node_address_and_challenge() {
        let signer = PeerProbeSigner::generate();
        let challenge = "01".repeat(32);
        let node = crate::ownership::NodeRec {
            node: "node_test".to_string(),
            expires_ms: u64::MAX,
            addr: "127.0.0.1:8080".to_string(),
            probe_public_key: signer.public_key_hex(),
            peer_protocol: crate::peer_auth::PROTOCOL_VERSION,
            ownership_index_generation: String::new(),
            load: crate::ownership::NodeLoad::default(),
        };
        let mut response = signer
            .respond(&node.node, &node.addr, &challenge)
            .unwrap();
        verify(&node, &challenge, &response).unwrap();

        response.advertise = "127.0.0.1:8081".to_string();
        assert!(verify(&node, &challenge, &response).is_err());
    }

    #[test]
    fn probe_challenge_is_fixed_lowercase_hex() {
        let signer = PeerProbeSigner::generate();
        assert!(signer.respond("node", "127.0.0.1:8080", &"00".repeat(32)).is_ok());
        assert!(signer.respond("node", "127.0.0.1:8080", &"AA".repeat(32)).is_err());
        assert!(signer.respond("node", "127.0.0.1:8080", "00").is_err());
    }
}
