...
assets are supported. Routes and vars are not; deploy those with Wrangler.\n\
...
fn read_project(path: &Path, root: &Path) -> anyhow::Result<Project> {
    ...
    if main.is_none() && (!do_classes.is_empty() || !sqlite_classes.is_empty()) {
        bail!("an asset-only project cannot declare Durable Objects");
    }
